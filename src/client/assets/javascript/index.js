...
async function handleCreateRace() {
	...
	if (!playerId || !trackId) {
		window.alert('You should choose a player or a track');
	} else {
		try {
			const race = await createRace(playerId, trackId);

			store = store.set('race_id', parseInt(race.ID) - 1);
			...
			console.error('problem with handlecreaterace');
		}
	}
}
...
function runRace(raceID) {
	return new Promise((resolve) => {
		...
		*/
		...
		const raceInterval = setInterval(update, 500);
	});
...
function handleSelectPodRacer(target) {
	...
	// add class selected to current targetd
